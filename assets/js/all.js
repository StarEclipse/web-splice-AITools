...
// swiper
var swiperM1 = new Swiper('.person-wriper', {
  spaceBetween: 24,
  // 分頁
  pagination: {
...
document.addEventListener('DOMContentLoaded', function () {
  var items = Array.from(document.querySelectorAll('#qaList li'));
  var buttons = items.map(function (item) {
    return item.querySelector('button');
  });
  var contents = items.map(function (item) {
    return item.querySelector('.overflow-hidden');
  });
  var icons = buttons.map(function (button) {
    return button.querySelector('.material-icons');
  });
...
document.addEventListener('DOMContentLoaded', function (event) {
  ...
  var sortDropDownItems = document.getElementById('sortDropDown').querySelectorAll('li');
...
var boxes = ['ani-box1', 'ani-box2', 'ani-box3'].map(function (id) {
